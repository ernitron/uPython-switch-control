...
from content import cb_status, cb_open, cb_setparam, cb_resetconf
#from contemp import cb_temperature, cb_temperature_json
...
class Server:
  ...
  def activate(self, timeout=0.3):
     # Attempts to aquire the socket and launch the server
     self.socket = socket.socket()
     self.socket.settimeout(timeout) # otherwise it will wait forever
     try:
  ...
  def wait_connections(self):
     ...
     if True:
         ...
         if r == None:
             ...
             header = httpheader(200, self.title)
             content = '<h2><a href="/">Restarting..."</a></h2></div>'
             self.http_send(header, content, self.footer)
             machine.reset()
         elif r['file'] != b'':
             header = httpheader(200, self.title)
             content = cb_open(r['file'])
         else:
             header = httpheader(404, self.title)
             content = error404

         # At end of loop just send, close and collect garbage
         self.http_send(header, content, self.footer)
         self.conn.close()
         gc.collect()
